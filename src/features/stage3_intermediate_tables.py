...
DEFAULT_OBSERVATION_START = datetime(2025, 11, 18, 0, 0, 0)
...
def _recency_hours(
    last_times: pa.ChunkedArray,
    reference_time: datetime,
) -> pa.Array:
    epoch = datetime(1970, 1, 1)
    reference_ns = int(
        (reference_time - epoch).total_seconds()
        * 1_000_000_000
    )
...
def build_user_features(
    ...
    grouped = grouped.append_column(
        "user_recency_hours",
        _recency_hours(
            grouped["user_last_behavior_time"],
            recency_reference_time,
        )
    )
    grouped = grouped.append_column(
        "user_fav_to_pv_rate",
...
def build_stage3_intermediate_tables(
    input_path: Path | str = DEFAULT_PARQUET_INPUT,
    *,
    observation_start: datetime = DEFAULT_OBSERVATION_START,
    cutoff_time: datetime,
    output_directory: Path | str,
) -> IntermediateTableBuildResult:
    """Build leakage-safe intermediate feature tables for one cutoff."""

    started_at = time.perf_counter()
    input_path = Path(input_path).resolve()
    output_directory = Path(output_directory).resolve()

    if cutoff_time < observation_start:
        raise ValueError("cutoff_time must not be earlier than observation_start")

    recency_reference_time = cutoff_time + timedelta(hours=1)
    ...
    builders = {
        "user_features": lambda table: build_user_features(
            table,
            recency_reference_time,
        ),
